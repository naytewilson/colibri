...
Final code commit: `39b258236fd978bd94125ffd32f4b01ce979db10`.
Final candidate binary sha256: `24a92a32ebb509d80bed5078a4dfb403879ed3f728fc5d761b9e7a1ab0911403`.
Baseline rebuild of clean f04359a reproduced the frozen promotion binary byte-exactly
...
1. New mode `ACQ_MODE_PRELOAD_NB` (+ `ACQ_WOULD_BLOCK` outcome). In
   ...
   wait-for-publish loop because no published slot remains immediately
   selectable (the captured wedge state had every slot unpublished/in-flight),
   a non-blocking preload now reports `ACQ_WOULD_BLOCK` and returns without
   touching cache state or counters. Published pinned slots remain subject to
   the pre-existing fallback eviction semantics and are not what triggers this
   new outcome.
...
true safe-reservation frontier — drains fire only when zero published slots
are selectable for reservation, never merely because `n == cap`.
...
Correction code commit `39b258236fd978bd94125ffd32f4b01ce979db10`:
`_Pragma("omp parallel for schedule(static) num_threads(W_)")`. W_ is consumed;
worker count <= 8 regardless of inherited OMP_NUM_THREADS; nl==1 stays serial;
nl>1 stays parallel. Zero new compiler warnings vs pristine f04359a baseline
census (all remaining warnings identical).
...
Performance confirmation (frozen serving protocol, same validation window,
final binary): batch_wall 12.06-12.45s with preload_io_sum 87.1-89.9s =>
7.2-7.3x effective load concurrency; live frozen-binary control arm in the
same window measured batch_wall 10.97s / io_sum 79.0s (same 7.2x ratio) while
frozen's own earlier passes spanned 9.95-10.64s — absolute drift tracks node
load, while the concurrency ratio remained unchanged. No sequentialization;
RSS flat ~10.8-10.9 GB serving profile.
...
1971084 evidence above is preserved unmodified. This receipt was subsequently
corrected docs-only by the controller to replace a stale intermediate commit
reference and bind the final candidate binary hash explicitly; the code-bearing
hotfix target remains `39b258236fd978bd94125ffd32f4b01ce979db10`.
